...
export const authService = {
  ...
  async socialLogin(accessToken: string, role?: string): Promise<LoginResponse> {
    try {
      ...
      console.log('Sending social login request data:', requestData)

      const response = await axios.post(`${API_BASE_URL}/auth/social-login`, requestData, {
        headers: {
          'Content-Type': 'application/json',
        },
      })
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (error.response) {
          throw new Error(
            error.response.data?.message || `Social login failed: ${error.response.status}`,
          )
        } else if (error.request) {
          throw new Error('No response from server. Check backend URL and network connection.')
        }
